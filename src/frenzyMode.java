...
// frenzy mode -  5 second timer for each level and the goal is to see how many blocks you can correctly identify as the off colored block
public class frenzyMode extends JFrame {
	...
	public frenzyMode(String version){
		currVersion = version;
		// set size
		setSize(600, 700);
			...
			public void run(){
				while (true){
					try {
						...
						String title = "Color Sleuth - Frenzy Mode | " + str;
						setTitle(title);
						sleep(1000L);
		...
		// timer in seconds
		JLabel timeLabel = new JLabel("TIME LEFT: 5");
		timeLabel.setFont(new Font("Arial Rounded MT Bold", Font.PLAIN, 15));
		timeLabel.setForeground(Color.BLACK);
		timeLabel.setBounds(150, 165, 200, 15);
		contentPane.add(timeLabel);
		
		Timer frenzyTimer = new Timer();
		TimerTask tt = new TimerTask() {
			// decrease timer by 1 second
			// if the timer is 0, the player automatically loses
			public void run(){
				if(count > 0) {
					count = count - 1;
					...
					endScreen lostScreen = new endScreen(score, "frenzy");   
        ...
        frenzyTimer.schedule(tt, 1000, 1000);
		...
		// increase score if correct button is clicked and reset the timer to 5 (take into account 1 second for processing so make it 6)
